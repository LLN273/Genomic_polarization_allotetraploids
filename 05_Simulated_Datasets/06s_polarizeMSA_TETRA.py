...
### 4. ID of tetraploid ancestral species
...
try:
    ...
    print('06s_polarizeMSA_TETRA.py [input.fasta] [No. species] [refSequence ID] [ancestral ID1] [ancestral ID2] \n')
    exit() 


try:
    RefSeqID = sys.argv[3]                  				      # Reference sequence ID
except:
    print('\n Error: provide the following arguments:')
    print('06s_polarizeMSA_TETRA.py [input.fasta] [No. species] [refSequence ID] [ancestral ID1] [ancestral ID2] \n')
    exit() 
...
try:
    AncestralID_1 = sys.argv[4]                  		       # ID first parental species  
except:
    print('\n Error: provide the following arguments:')
    print('06s_polarizeMSA_TETRA.py [input.fasta] [No. species] [refSequence ID] [ancestral ID1] [ancestral ID2] \n')
    exit() 
...
try:
    AncestralID_2 = sys.argv[5]                  			     # ID second parental species
except:
    print('\n Error: provide the following arguments:')
    print('06s_polarizeMSA_TETRA.py [input.fasta] [No. species] [refSequence ID] [ancestral ID1] [ancestral ID2] \n')
    exit() 
...
for tag in INfasta_species_CLEAN :
   ...
   species_list.append(tag[:(len(tag) - 4)])	  #remove "_0_0" from eg "14_0_0" ["14" is the species name]
...
###################################################### Create diploid and allotetraploid sequences



## Create allotetraploid sequence, IUPAC coded
...
for key, value in DIPdic.items():
    ...
    DEP_IDs.append(tag[:(len(tag) - 4)])	      #remove "_0_0" from eg "14_0_0" ["14" is the species name]
...
## add allotetraploid to dictionary
...
REFG_ID =  RefSeqID + 'R'
DICT_seq[REFG_ID] = DICT_seq.pop(RefSeqID)
...
###################################################### Unfold allotetraploid and reference sequence
...
# remove unpolarized allopolyploid from dictionary (polarized allopolyploid will be added in the next step)
POLY_ID = '>' + POLYPID
...
## Count differences between polarized and non-polarized allopolyploid sequence
